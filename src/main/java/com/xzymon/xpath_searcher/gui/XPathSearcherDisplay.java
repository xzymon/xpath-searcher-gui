...
public class XPathSearcherDisplay extends JTextPane implements StateHolder, ParserListener, XPathSearchingListener{
	...
	public boolean loadHTMLStream(InputStream is) {
		boolean result = false;
		InputStream preparedIs = null;
		
		try{
			preparedIs = prepareHtml(is, standardCssElementsToRemove);
			result = loadStream(preparedIs);
		} catch (IOException e1) {
			e1.printStackTrace();
		} finally {
	...
	private InputStream prepareHtml(InputStream srcStream, List<String> elementTypesToRemove) throws IOException{
		...
		if(avail>0){
			...
			DocumentTreeRepresentation docTree = new DocumentTreeRepresentation(new ByteArrayInputStream(savedStream), XmlPreprocessingMode.ASSUME_HTML,
					htmlDefaultPolicies);
